...
## 📋 Document Naming Convention for Auto-Categorization

The app automatically sorts your documents into categories (Aadhaar, PAN, Passport, etc.) based on **keywords in the filename**. Here's how it works:
...
>
> **Examples — files that go to Other Documents (keyword missing or not matching as a separate word):**
...
> - `doc_123.pdf` → no recognizable keyword in the filename
...
> **Just name your files with the document type in the filename.** For short abbreviations like PAN, RC, DL, PUC — use them as separate words (with underscores, spaces, or hyphens around them). Everything else just needs to contain the keyword anywhere.
